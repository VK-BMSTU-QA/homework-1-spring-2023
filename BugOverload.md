...
2. Надписи соответствуют полям отправляемого json
3. Если пароль больше 5 символов, и он содержит большую и маленькую буквы, то сообщение об ошибке пропадает, при условии, что оно появилось до этого. Аналагично и для других полей.
...
4. Текст ошибки имеет достаточный для чтения размер на всех экранах_**`BUG`**_: при ширине экрана меньше 600px текст становится нечитаемым
...
2. Фото профиля загружается на сервер, обновлясь на клиенте_**`BUG`**_: не получилось загрузить фото, в тесте использовались фотографии разных размеров
3. Дата регистрации указана в формате дд.мм.гггг
4. Если оценок или рецензий нет, то в поле оценки надпись: нет оценок, а в графе рецензии: нет рецензий
5. После того, как оценили фильм, в профиле увеличилось количество оценок на 1
6. После публикации рецензии, общее количество рецензий в профиле увеличилось на 1_**`BUG`**_: не удалось проверить, так как не получается опубликовать рецензии
7. При удалении оценки, общее количество оценок в профиле уменьшится на 1
8. При ширине экрана < 526px основная информация перемещается под профиль
9. При наведение на кнопку "Выбрать файл", её задний фон осветляется
...
5. При нажатии на фото персоны происходит переход на страницу персоны. URL: https://movie-gate.online/person/1/
...
5. Слайдер позволяет прокручивать фильмы (Аналогично Главной странице [↑](#главная))
...
5. При нажатии на компонент - фильм, будет редирект на [страницу фильма](https://movie-gate.online/film/1/)
...
8. При нажатии на аватар автора будет редирект на его профиль
